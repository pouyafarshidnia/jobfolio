 ...
 * Methods
 */
function updateModalData(application) {
...
<template>
  ...
  <div class="p-5">
    <!-- Header -->
    <Header @resetModal="resetModal" :show="showModal" :data="modalData" :countries="countries"
    ...
    <Table>
      ...
      <TableBody>
        <TableRow v-for="(application, index) in list.data" :key="application.id">
          <TableData class="font-medium whitespace-nowrap text-pink-800 dark:text-gray-100">
            {{ index + 1 }}
          </TableData>

          <TableData class="flex items-center gap-3 whitespace-nowrap text-gray-700 dark:text-gray-300">
            <img :src="application.country.flag.thumbnail" alt="" class="h-5 w-5 rounded" />
            {{ application.country.flag.label }}
          </TableData>

          <TableData class="font-bold whitespace-nowrap text-gray-700 dark:text-gray-300">
            {{ application.company }}
          </TableData>

          <TableData class="whitespace-nowrap text-gray-700 dark:text-gray-300">
          ...
          <TableData class="whitespace-nowrap text-gray-500 dark:text-gray-400">
          ...
          <TableData class="whitespace-nowrap">
            <span :class="'badge badge-' + application.status.color">
              {{ application.status.label }}</span>
          </TableData>

          <TableData class="whitespace-nowrap text-gray-500 dark:text-gray-400">
            <a v-if="application.link.url" :href="application.link.url">
              <component :is="linkIcons[application.link.icon]" class="h-4 w-4 text-gray-600" />
            </a>
            <span v-else>
              <component :is="linkIcons[application.link.icon]" class="h-4 w-4 text-gray-400" />
            </span>
          </TableData>

          <TableData class="text-right whitespace-nowrap">
            <!-- Process -->
            <TooltipProvider :delay-duration="0" v-if="application.actions.processable">
              <Tooltip>
                <TooltipTrigger as-child>
                  <ActionLink :href="process(application)"
            ...
            <!-- Approve -->
            <TooltipProvider :delay-duration="0" v-if="application.actions.approvable">
              <Tooltip>
                <TooltipTrigger as-child>
                  <ActionLink :href="approve(application)"
            ...
            <!-- Reject -->
            <TooltipProvider :delay-duration="0" v-if="application.actions.rejectabale">
              <Tooltip>
                <TooltipTrigger as-child>
                  <ActionLink :href="reject(application)"
            ...
            <TooltipProvider :delay-duration="0">
              <Tooltip>
                <TooltipTrigger as-child>
                  <button type="button" @click="updateModalData(application)"
    ...
    <!-- PAgination -->
    <Pagination :list="list.meta" :nextUrl="list.links.next" :prevUrl="list.links.prev"></Pagination>
  </div>
</template>
